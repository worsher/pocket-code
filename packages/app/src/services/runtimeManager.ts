...
import { ensureMobileStorageLayout } from "./workspaceResolver";
...
function getLegacyRootfsDir(): Directory {
  return new Directory(Paths.document, "rootfs");
}

function getLegacyManifestFile(): File {
  return new File(Paths.document, INSTALLED_MANIFEST);
}

function useLegacyRuntimeLayout(): boolean {
  return getLegacyManifestFile().exists || new File(getLegacyRootfsDir(), "bin/sh").exists;
}

function getRuntimeDir(): Directory {
  if (useLegacyRuntimeLayout()) return new Directory(Paths.document);
  return new Directory(ensureMobileStorageLayout(), "runtime");
}

function getRootfsDir(): Directory {
  return useLegacyRuntimeLayout() ? getLegacyRootfsDir() : new Directory(getRuntimeDir(), "rootfs");
}

function getManifestFile(): File {
  return useLegacyRuntimeLayout()
    ? getLegacyManifestFile()
    : new File(getRuntimeDir(), INSTALLED_MANIFEST);
}

function getTarGzFile(): File {
  return new File(getRuntimeDir(), "alpine-minirootfs.tar.gz");
}
...
export function getRuntimeRootfsNativePath(): string {
  return nativePath(getRootfsDir().uri);
}

export function getRuntimeTempNativePath(): string {
  return `${nativePath(Paths.cache.uri)}/proot-tmp`;
}
...
export async function bootstrapRootfs(onProgress?: ProgressCallback): Promise<void> {
  ...
  const manifestFile = getManifestFile();
  manifestFile.write(JSON.stringify({ version: ALPINE_VERSION, packages: [] }));
 ...
 */
export function buildProotCommand(cmd: string, cwd: string, workspaceRoot?: string): string {
  const mod = requireNativeModule("PocketTerminalModule");
  const nativeLibDir: string = mod.getNativeLibDir?.() ?? "";
  const prootBin = `${nativeLibDir}/libproot.so`;
  const prootLoaderBin = `${nativeLibDir}/libproot-loader.so`;
  const prootTmpDir = getRuntimeTempNativePath();
  const rootfsDir = getRuntimeRootfsNativePath();
  ...
  const workspaceDir = nativePath(workspaceRoot ?? new Directory(Paths.document, "workspace").uri);
