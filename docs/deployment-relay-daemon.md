...
|---|---|---|
| `PORT` | 监听端口 | `3200` |
| `RELAY_SECRET` | **强烈建议设置**。设了之后 Daemon 注册需 HMAC-SHA256 签名(防陌生 daemon 接入)。Relay 与 Daemon 必须用**同一个值** | 不验证 |
...
| `RELAY_SECRET` | 必须**与 Relay 的一致**(若 Relay 设了) | 无 |
...
- **务必设 `RELAY_SECRET`**（Relay 与 Daemon 同值），否则任意 daemon 可向你的 Relay 注册。
