...
**要点：**
1. `createDeviceBackend(opts: { projectId?: string; execTool: (name, args) => Promise<unknown>; workspaceRoot: string }): RuntimeBackend`：readFile/writeFile/listFiles → `localFileSystem` 的 readLocalFile/writeLocalFile/listLocalFiles（路径基 `opts.workspaceRoot`——调用方算好后与 `runAgentLoop` 的 `workspace` 传同一个值,单一真相;不在内部重新调用 `getProjectWorkspaceRoot`/`getDefaultWorkspace` 求根,见下方 T10 审查轮更正注记；返回值形状适配 RuntimeBackend——对照 localFileSystem 实际签名）；exec → `execTool("runCommand", {command,...})` 解析其 `{success,stdout,stderr,error}` 回 ExecResult（success:false 且无 exitCode → exitCode 1）；startProcess/stopProcess → `execTool("runInBackground"/"stopProcess", ...)`（保持 geek 现有能力）。
2. `useAgent.sendGeekMessage`：`runGeekLoop` 调用替换为：

> **实现更正（T10 审查轮）**：下方样例中 `workspace: "/"` 在实现中被证明有缺陷——core 的
> `safePath` 守卫用 `full.startsWith(ws + "/") || full === ws` 判定越界，`ws` 归一化为 `"/"` 时
> 拼出 `"/" + "/"` = `"//"`，导致几乎所有相对路径都不满足 `startsWith("//")`，从而被误判为路径
> 穿越、全体相对路径遭拒。最终方案放弃了这个哨兵路径，改为：`workspace` 传**真实设备工作区根**
> （`getProjectWorkspaceRoot(projectId) ?? getDefaultWorkspace()`），并在 `DeviceBackend` 内做
> **双形状 cwd 翻译**——因为 core 工具经两条不同路径把 cwd 传给 `backend.exec`：一部分（如
> `runCommandTool`、`gitCloneTool`）透传**原始** `workspace` 字符串（真机上形如
> `file:///data/.../workspace`，未经归一化），另一部分（8 个 git 工具经 `resolveGitCwd`）传
> `safePath(root, ...)` 产出的**归一化后**绝对路径（core 的纯字符串 `resolvePosix` 会把
> `file:///...` 的三斜杠折叠成 `/file:/...`，与原始根不是同一字符串）。`DeviceBackend.exec`
> 因此需要同时比较"原始根"与"归一化根"两种前缀形状，命中任一个才正确剥离为相对路径转发给
> `execTool`；样例代码已同步改为真实根 + 双形状翻译，实现见
> `packages/app/src/services/deviceBackend.ts`（`normalizedRoot`/`toRelativePath`/`exec` 内的
> 三段 if/else if/else）。

```ts
const workspaceRoot = getProjectWorkspaceRoot(projectIdRef.current) ?? getDefaultWorkspace();
const { messages: nextHistory } = await runAgentLoop({
  ...
  backend: createDeviceBackend({ projectId: projectIdRef.current, execTool: executeTool, workspaceRoot }),
  workspace: workspaceRoot,    // 真实设备工作区根(单一真相,与 DeviceBackend 的 workspaceRoot 同值)
