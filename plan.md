...
3. ~~**P12–P16:五项可靠性/自治改进(借鉴 kimi-code)**~~(✅ 2026-07-21 全部完成:P12 loop 结构化 stopReason → P13 瞬时错误重试+媒体降级 → P14 事件流 seq+重连补发+断线续跑 → P15 上下文压缩 → P16 Goal 模式(自治多轮,/goal 下发 + daemon 续跑 + 状态卡片)。spec 见 specs/2026-07-21-kimi-code借鉴五项改进-design.md;plans 见 plans/2026-07-21-p12-p13-*/p14-*/p15-*/p16-*。真机冒烟后置——各 plan 的手动验收清单)。
